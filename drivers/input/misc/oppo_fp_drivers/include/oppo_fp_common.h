...
typedef struct fp_module_gpio_config_info {
	int gpio_id_config_list
		[3]; /*gpio_id_config_list[0]-->gpio ID1, and so on; if a ID is not used, then value == -1*/
	int fp_vendor_chip;
	char fp_chip_module[FP_ID_MAX_LENGTH];
	char engineermode_menu_config
		[ENGINEER_MENU_SELECT_MAXLENTH]; /*it depends on macro ENGINEER_MENU_XXX in oppo_fp_common.c*/
} fp_module_config_t;
