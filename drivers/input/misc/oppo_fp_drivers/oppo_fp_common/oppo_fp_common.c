 ...
 **  Wenhao         2024/06/14     port to lineage-19.1, replace project ID detection with Kconfig
 ************************************************************************************/
...
/*
 * fp_module_config_list: maps GPIO ID pin combinations to fingerprint chip types.
 * The GPIO reading scheme varies by board - see the corresponding fp_register_proc_fs() logic.
 */

#if IS_ENABLED(CONFIG_MACH_OPPO_R11)
/* R11 (16051): FPC 1140 and 1260, multiple module vendors */
...
#elif IS_ENABLED(CONFIG_MACH_OPPO_R11S) || IS_ENABLED(CONFIG_MACH_OPPO_PAAM00)
/* R11s (17011/17015) and R15 Pro (17081/17085): FPC + Goodix capacitive mix */
fp_module_config_t fp_module_config_list[] = {
...
#elif IS_ENABLED(CONFIG_MACH_OPPO_PBCM10)
/* R15x/K1 (18005/18321/18323): Silead Optical + Goodix Optical */
fp_module_config_t fp_module_config_list[] = {
...
#else
/* Default: FPC + Goodix 5298 (fallback for unspecified boards) */
fp_module_config_t fp_module_config_list[] = {
...
#if IS_ENABLED(CONFIG_MACH_OPPO_R11S) || IS_ENABLED(CONFIG_MACH_OPPO_PAAM00)
    /* R11s & R15 Pro: use gpio_id1, id2, id3 */
...
#elif IS_ENABLED(CONFIG_MACH_OPPO_PBCM10)
    /* R15x/K1: use gpio_id2 only (id0/id1 fixed) */
    ret = fp_request_named_gpio(fp_data, "oppo,fp-id2",
            &fp_data->gpio_id2);
    if (ret) {
        ret = FP_ERROR_GPIO;
...
#else
    /* Default (R11 and others): use gpio_id0, id1, id2 */
...
#if IS_ENABLED(CONFIG_MACH_OPPO_R11S) || IS_ENABLED(CONFIG_MACH_OPPO_PAAM00)
    /* R11s & R15 Pro: use id1, id2, id3 from GPIOs */
...
#elif IS_ENABLED(CONFIG_MACH_OPPO_PBCM10)
    /* R15x/K1: id0=1, id1=1 fixed, id2 from GPIO */
    fp_data->fp_id0 = 1;
    fp_data->fp_id1 = 1;
    fp_data->fp_id2 = gpio_get_value(fp_data->gpio_id2);

#else
    /* Default (R11): read all three */
...
#if defined(CONFIG_OPTICAL_IRQ_HANDLER)
