...
static int synaptics_get_touch_points(void *chip_data, struct point_info *points, int max_num)
{
    ...
    buf = kzalloc(8 * fingers_to_process, GFP_KERNEL);
    if (buf == NULL) {
        TPD_INFO("buf kzalloc error\n");
        return -ENOMEM;
    }
...
static void synaptics_auto_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
    ...
    buffer_rx = kzalloc(syna_testdata->RX_NUM, GFP_KERNEL);
    buffer_tx = kzalloc(syna_testdata->TX_NUM, GFP_KERNEL);
    if (!buffer_rx || !buffer_tx) {
            TPD_INFO("buffer_rx/buffer_tx kzalloc error\n");
            kfree(buffer_rx);
            kfree(buffer_tx);
            kfree(raw_data);
            return;
...
static void synaptics_bootup_test(void *chip_data, const struct firmware *fw, struct monitor_data *mon_data, struct hw_resource *hw_res)
{
    ...
    buffer_rx = kzalloc(hw_res->RX_NUM, GFP_KERNEL);
    buffer_tx = kzalloc(hw_res->TX_NUM, GFP_KERNEL);
    if (!buffer_rx || !buffer_tx) {
        TPD_INFO("buffer_rx/buffer_tx kzalloc error\n");
        kfree(buffer_rx);
        kfree(buffer_tx);
