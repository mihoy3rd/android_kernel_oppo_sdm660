	...
	void *argp)
{
	...
	case CFG_ACTUATOR_INIT:
		...
		if (a_ctrl->firmware_load) {
			...
			set_fs(KERNEL_DS);
			offsize = 0;
			size = vfs_read(mfile, vendor_id,
				sizeof(vendor_id) - 1, &offsize);
			if (size < 0) {
			...
			if (strcmp(vendor_id, "3") == 0) {
				...
				set_fs(KERNEL_DS);
				offsize = 0;
				size = vfs_read(mfile, data_info,
					sizeof(data_info) - 1, &offsize);
				if (size < 0) {
				...
				set_fs(KERNEL_DS);
				offsize = 0;
				size = vfs_read(mfile, vcm_id,
					sizeof(vcm_id) - 1, &offsize);
				if (size < 0) {
