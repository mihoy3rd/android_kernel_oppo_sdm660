...
power_down:
	power_down_rc = msm_camera_power_down(power_info,
		e_ctrl->eeprom_device_type,
		&e_ctrl->i2c_client);
	if (power_down_rc < 0) {
		pr_err("%s:%d Power down failed rc %d\n",
			__func__, __LINE__, power_down_rc);
		if (!rc)
			rc = power_down_rc;
	}
	...
	void *argp)
{
	...
	case CFG_EEPROM_GET_MM_INFO:
		...
		cdata->cfg.get_cmm_data.cmm_support =
			e_ctrl->eboard_info->cmm_data.cmm_support;
		cdata->cfg.get_cmm_data.cmm_compression =
			e_ctrl->eboard_info->cmm_data.cmm_compression;
		cdata->cfg.get_cmm_data.cmm_size =
			e_ctrl->eboard_info->cmm_data.cmm_size;
		break;
	case CFG_EEPROM_INIT:
		if (e_ctrl->userspace_probe == 0) {
			pr_err("%s:%d Eeprom already probed at kernel boot\n",
...
static int msm_eeprom_read_moduleinfo(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	memptr = e_ctrl->cal_data.mapdata;
	if (!memptr ||
		e_ctrl->cal_data.num_data <= EEPROM_REG_SENSOR_ID_H)
		return -EINVAL;
...
static int msm_eeprom_read_vcminfo(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	memptr = e_ctrl->cal_data.mapdata;
	if (!memptr ||
		e_ctrl->cal_data.num_data <= EEPROM_REG_VCM_ID_H)
		return -EINVAL;
...
static int msm_eeprom_read_lensid(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	memptr = e_ctrl->cal_data.mapdata;
	if (!memptr ||
		e_ctrl->cal_data.num_data <= EEPROM_REG_LENS_ID_H)
		return -EINVAL;
...
static int msm_eeprom_compare_dateinfo(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	memptr = e_ctrl->cal_data.mapdata;
	if (!memptr ||
		e_ctrl->cal_data.num_data <= EEPROM_REG_MODULE_YEAR_H)
		return -EINVAL;
...
static int msm_eeprom_proc_init(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	if (e_ctrl->position < 0 || e_ctrl->position > 2) {
		pr_err("%s:%d: invalid eeprom position %d\n",
			__func__, __LINE__, e_ctrl->position);
		return -EINVAL;
	}

	if (eeprom_probe_info & BIT(e_ctrl->position)) {
	...
	eeprom_probe_info |= BIT(e_ctrl->position);
    return 0;
}
...
/*Added by Jinshui.Liu@Camera 20160821 for [module vendor info]*/
	rc = of_property_read_u32(of_node, "position", &temp);
	if (rc < 0 || temp > 2) {
		e_ctrl->position = -1;
		pr_err("%s:%d invalid or missing eeprom position\n",
			__func__, __LINE__);
	} else {
		e_ctrl->position = temp;
	}
