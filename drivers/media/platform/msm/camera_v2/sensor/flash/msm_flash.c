...
{
	int32_t rc = 0;
	bool power_settings_compat = false;
	...
	if (!flash_init_info || !flash_init_info->power_setting_array) {
...
#ifdef CONFIG_COMPAT
		if (is_compat_task()) {
			memset(&flash_ctrl->power_setting_array, 0,
				sizeof(flash_ctrl->power_setting_array));
		...
		msm_flash_copy_power_settings_compat(
			...
			kfree(power_setting_array32);
			power_settings_compat = true;
		}
#endif
		if (!power_settings_compat &&
			copy_from_user(&flash_ctrl->power_setting_array,
			(void __user *)flash_init_info->power_setting_array,
	...
	if (flash_init_info->settings) {
		settings = memdup_user((void __user *)flash_init_info->settings,
			sizeof(*settings));
		if (IS_ERR(settings))
	...
	(flash_ctrl->power_info.power_down_setting_size > MAX_POWER_CONFIG)) {
		pr_err("%s:%d invalid power setting size=%d size_down=%d\n",
			...
			flash_ctrl->power_info.power_down_setting_size);
		rc = -EINVAL;
		goto msm_flash_i2c_free_settings;
	}
...
msm_flash_i2c_power_down:
	msm_camera_power_down(&flash_ctrl->power_info,
		flash_ctrl->flash_device_type,
		&flash_ctrl->flash_i2c_client);
msm_flash_i2c_free_settings:
	kfree(settings);
	return rc;
}
