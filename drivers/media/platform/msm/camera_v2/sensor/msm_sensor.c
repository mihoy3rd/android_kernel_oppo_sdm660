...
static void register_device_info(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (position < 0 || position > 2) {
		pr_err("%s:%d: invalid camera position %d\n",
			__func__, __LINE__, position);
		return;
	}

	if (deviceInfo_register_value & BIT(position)) {
	...
	if (i < size_of_module_list) {
		...
		CDBG("%s:%d: will register device %s %s\n",
			__func__, __LINE__, cam_module_info[i].string,
			s_ctrl->sensordata->sensor_name);
		register_device_proc(position_string,
			(char *)s_ctrl->sensordata->sensor_name,
			cam_module_info[i].string);

		deviceInfo_register_value |= BIT(position);
	}
}
