...
static DEFINE_MUTEX(sensor_fuseid_lock);
static char sensor_fuseid[FUSEID_MAX_LENGTH];
	...
	size_t len, loff_t *data)
{
	...
	mutex_lock(&sensor_fuseid_lock);
	memcpy(sensor_fuseid, buf, len + 1);
	mutex_unlock(&sensor_fuseid_lock);
	...
	size_t len, loff_t *data)
{
	...
	mutex_lock(&sensor_fuseid_lock);
	rc = simple_read_from_buffer(buff, len, data, sensor_fuseid,
		strnlen(sensor_fuseid, sizeof(sensor_fuseid)));
	mutex_unlock(&sensor_fuseid_lock);
