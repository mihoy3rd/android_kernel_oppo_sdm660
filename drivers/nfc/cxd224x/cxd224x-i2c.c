			 ...
			 const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, "wenjie cxd224x_probe\n");
...
/* so need Distinguish 17088,
* Operator_Version equals 6
 */
	if (is_project(OPPO_17085) && (get_Operator_Version() == 6)) {
		dev_err(&client->dev, "normal cxd224x_probe Project[17085] Operator[6] mean 17088 \n");
	} else {
		dev_err(&client->dev, "wrong cxd224x_probe get_project[%d] get_Operator_Version [%d] \n",get_project(),get_Operator_Version());
		return -ENODEV;
	}
...
{
    int res = 0 ;
    res = i2c_add_driver(&cxd224x_driver);
	
	printk("Entry cxd224x_dev_init res = %d",res);
