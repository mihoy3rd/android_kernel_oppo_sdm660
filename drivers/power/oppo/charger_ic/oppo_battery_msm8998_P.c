...
#ifndef CONFIG_MACH_OPPO_SDM660
...
#ifdef CONFIG_MACH_OPPO_SDM660//Fanhong.Kong@PSW.BSP.CHG,add 2017/12/02 for shipmode  stm6620
    if (g_oppo_chip) {
...
static int smbchg_get_chargerid_volt(struct oppo_chg_chip *chip)
{
	...
	rc = iio_read_channel_processed(chip->pmic_spmi.pm660_vadc_dev, &val);
	if (rc < 0) {
		chg_err("unable to read pm660_vadc_dev rc = %d\n", rc);
		return 0;
	}
	chargerid_volt = val / 1000;
...
if (of_find_property(oppo_chip->dev->of_node, "qcom,pm660chg-vadc", NULL)) {
	oppo_chip->pmic_spmi.pm660_vadc_dev = iio_channel_get(oppo_chip->dev, "pm660chg");
	if (IS_ERR(oppo_chip->pmic_spmi.pm660_vadc_dev)) {
		...
		if (rc != -EPROBE_DEFER)
			chg_err("Couldn't get iio channel rc=%d\n", rc);
		else {
			chg_err("Couldn't get iio channel, try again...\n");
			return -EPROBE_DEFER;
		}
	}
}
