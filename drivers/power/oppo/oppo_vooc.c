...
static void oppo_vooc_awake_init(struct oppo_vooc_chip *chip)
{
	...
	chip->vooc_ws = wakeup_source_register(chip->dev, "vooc_wake_lock");
}
...
static void oppo_vooc_watchdog(struct timer_list *t)
{
        struct oppo_vooc_chip *chip = from_timer(chip, t, watchdog);
