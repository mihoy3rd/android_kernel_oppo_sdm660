...
/* Yongzhi.Zhang@PSW.MM.AudioDriver.FTM.1408074, 2018/06/05,
...
char ftm_SpeakerCalibration_v6[17] = "calibration_ok";
static char ftm_path[15] = "open_path_ok";
char ftm_spk_resistance_v6[24] = "speaker_resistance_ok";
...
/* /sys/kernel/debug/ftm_tfa98xx */
	...
	n += scnprintf(buffer + n, size - n, "%s ", ftm_SpeakerCalibration_v6);
	n += scnprintf(buffer + n, size - n, "%s ", ftm_path);
	n += scnprintf(buffer + n, size - n, "%s ", ftm_spk_resistance_v6);
	n += scnprintf(buffer + n, size - n, "%s ", ftm_tfa98xx_flag);
	n += scnprintf(buffer + n, size - n, "%d ", ftm_mode_v6);
...
static enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	if (ftm_mode_v6 == BOOT_MODE_FACTORY) {
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	/* Yongzhi.Zhang@PSW.MM.AudioDriver.HAL.1410538, 2018/06/02,
	 * add for keeping the calibration debugfs path the same as old projects */
	scnprintf(name, MAX_CONTROL_NAME, "%s-36", i2c->name);
		...
		if (do_sync) {
			...
			list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
				...
				if (!g_speaker_resistance_fail) {
					...
					tfa_dev_set_state(tfa98xx->tfa, TFA_STATE_UNMUTE);
				/* Yongzhi.Zhang@PSW.MM.AudioDriver.SmartPA, 2018/08/16,
				 * modify for setting mute when resistance is out of range */
				} else {
					pr_info("set mute state\n");
...
/* xiang.fei@MM.AudioDriver.Codec, 2018/03/12, Modify for Add for gpio */
	...
	ftm_mode_v6 = get_boot_mode();
	pr_info("ftm_mode_v6=%d\n", ftm_mode_v6);
...
static int __init tfa98xx_i2c_init(void)
{
	...
	/* Yongzhi.Zhang@PSW.MM.AudioDriver.SmartPA, 2018/07/06,
	 * modify for TFA9894 for 18005 & 18323 */
	if ((is_project(OPPO_18316)) || (is_project(OPPO_18005)) || (is_project(OPPO_18323))) {
