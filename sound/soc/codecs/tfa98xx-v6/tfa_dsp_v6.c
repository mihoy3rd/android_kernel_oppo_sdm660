...
extern int ftm_mode_v6;
extern char ftm_SpeakerCalibration_v6[17];
extern char ftm_spk_resistance_v6[24];
	...
	if(err == Tfa98xx_Error_Ok) {
		...
		if ((tfa->mohm[0] < 6000) || (tfa->mohm[0] > 10500)) {
			...
			if (ftm_mode_v6 == BOOT_MODE_FACTORY) {
				strcpy(ftm_spk_resistance_v6, "speaker_resistance_fail");
			}
...
enum Tfa98xx_Error tfaRunWaitCalibration_v6(struct tfa_device *tfa, int *calibrateDone)
{
	...
	*calibrateDone = 0;
	...
	if (*calibrateDone != 1) {
		/*Jianfeng.Qiu@PSW.MM.AudioDriver.FTM.1226731, 2018/05/12, Add for FTM*/
		if (ftm_mode_v6 == BOOT_MODE_FACTORY) {
			strcpy(ftm_SpeakerCalibration_v6, "calibration_fail");
		}
