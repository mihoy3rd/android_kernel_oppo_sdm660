...
extern int afe_dsm_get_libary_info(uint8_t *payload);
		...
		size_t count, loff_t *ppos)
{
	...
	rc = afe_dsm_rx_set_params(payload, count);
	if (!rc)
		rc = count;
exit:
	mutex_unlock(&dsm_lock);
...
static void max98927_misc_deregister(void *data)
{
					...
					loff_t *ppos)
{
	...
	afe_dsm_get_libary_info((uint8_t *)payload);
...
{
	u32  ret = 0;
	//struct snd_soc_component *codec = w->component;
...
static int max98927_reset(struct i2c_client *i2c,
			  struct max98927_priv *max98927)
{
	int ret;

	max98927->reset_gpio_l = of_get_named_gpio(i2c->dev.of_node,
						   "maxim,98927-reset-gpio", 0);
	if (max98927->reset_gpio_l == -EPROBE_DEFER)
		return -EPROBE_DEFER;
	if (!gpio_is_valid(max98927->reset_gpio_l)) {
		dev_err(&i2c->dev, "invalid reset GPIO: %d\n",
			max98927->reset_gpio_l);
		return max98927->reset_gpio_l < 0 ?
			max98927->reset_gpio_l : -EINVAL;
	}

	ret = devm_gpio_request_one(&i2c->dev, max98927->reset_gpio_l,
				    GPIOF_OUT_INIT_LOW, "max_98927_reset");
	if (ret) {
	...
	msleep(10);
	gpio_set_value_cansleep(max98927->reset_gpio_l, 1);
	msleep(5);

	return 0;
}

static void max98927_disable_regulator(void *data)
{
...
static void max98927_clear_i2c_states(void *data)
{
	i2c_states = 0;
}
...
{
	struct max98927_priv *max98927;
	unsigned long channel = id ? id->driver_data : MAX98927L;
	int value, i, ret = 0;
	...
	if (!of_property_read_u32(i2c->dev.of_node, "interleave_mode", &value)) {
		if (value > 1) {
			dev_err(&i2c->dev, "invalid interleave mode: %d\n", value);
			return -EINVAL;
		}
	 ...
	 */
	max98927->max989xx_vdd = devm_regulator_get(&i2c->dev,
						    "max989xx_vdd");
	if (IS_ERR(max98927->max989xx_vdd)) {
		dev_err(&i2c->dev, "failed to get max989xx_vdd\n");
	...
	if (ret) {
		dev_err(&i2c->dev, "failed to enable max989xx_vdd: %d\n", ret);
		return ret;
	}
	ret = devm_add_action_or_reset(&i2c->dev,
				       max98927_disable_regulator,
				       max98927->max989xx_vdd);
	if (ret)
		return ret;
	...
	max98927->regmap[channel] =
		devm_regmap_init_i2c(i2c, &max98927_regmap);
	if (IS_ERR(max98927->regmap[channel])) {
		ret = PTR_ERR(max98927->regmap[channel]);
		dev_err(&i2c->dev,
			"failed to allocate channel %lu regmap: %d\n",
			channel, ret);
		return ret;
	} else {    //below initialize the register by mode and chip status.
		if (check_max98927_presence(max98927->regmap[channel])) {
			presence = (1 << channel);
			if (max98927->mono_stereo == 0) {
				...
				for (i = 0;i < sizeof(reg_channel_map[0])/sizeof(reg_channel_map[0][0]);i++)
					regmap_write(max98927->regmap[channel], reg_channel_map[0][i][0], reg_channel_map[0][i][1]);
			} else if (max98927->mono_stereo & presence) {
				i2c_states |= presence;    //mark this chip, then app can address it.
				for (i = 0;i < sizeof(reg_channel_map[channel+1])/sizeof(reg_channel_map[channel+1][0]);i++)
					regmap_write(max98927->regmap[channel],
							reg_channel_map[channel+1][i][0], reg_channel_map[channel+1][i][1]);
			}
			for (i = 0;i < sizeof(reg_common_map)/sizeof(reg_common_map[0]);i++)
				regmap_write(max98927->regmap[channel], reg_common_map[i][0], reg_common_map[i][1]);

			if (max98927->interleave_mode)
				regmap_write(max98927->regmap[channel], MAX98927_PCM_Tx_Channel_Sources_B, 0x20);

		}
	}

	if (!presence || !i2c_states) {
		dev_err(&i2c->dev, "MAX98927 detection failed at 0x%x\n",
			i2c->addr);
		return -ENODEV;
	}

	ret = devm_add_action_or_reset(&i2c->dev,
				       max98927_clear_i2c_states, NULL);
	if (ret)
		return ret;
	...
	if (ret) {
		dev_err(&i2c->dev, "failed to register DSM misc device: %d\n",
			ret);
		return ret;
	}
	ret = devm_add_action_or_reset(&i2c->dev,
				       max98927_misc_deregister,
				       &dsm_ctrl_miscdev);
	if (ret)
		return ret;
#endif
