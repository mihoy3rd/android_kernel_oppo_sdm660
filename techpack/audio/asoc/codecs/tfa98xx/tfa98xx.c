...
static void vol_gradual_change(struct work_struct *work)
{
	...
	val = snd_soc_component_read32(tfa98xx->component, TFA98XX_AUDIO_CTR);
	...
	if((val & TFA98XX_AUDIO_CTR_VOL_MSK) <= 0x4000 ){
	    ...
	    if(0 == tfa98xx_profile) {
            ...
            ret = snd_soc_component_write(tfa98xx->component,
                    TFA98XX_AUDIO_CTR,
                    (0x0700 & TFA98XX_AUDIO_CTR_VOL_MSK));
	    } else {
            ret = snd_soc_component_write(tfa98xx->component,
                    TFA98XX_AUDIO_CTR,
	    ...
	    #else
        ret = snd_soc_component_write(tfa98xx->component,
                TFA98XX_AUDIO_CTR, val & ~TFA98XX_AUDIO_CTR_VOL_MSK);
	    #endif
...
{
    if (tfa98xx->handle < 0 ||
        tfa98xx->handle >= ARRAY_SIZE(tfa98xx_devices) ||
        tfa98xx_devices[tfa98xx->handle] != tfa98xx)
        return;

    if (tfa98xx_registered_handles > 0)
        tfa98xx_registered_handles--;
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
    ...
    if (ret) {
        tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_FAIL;
        dev_err(tfa98xx->dev, "failed to load %s: %d\n", fw_name, ret);
        return 0;
    }
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	val |= TFA98XX_AUDIO_CTR_VOL_MSK;
	pr_debug("val : %x", val);
	snd_soc_component_write(tfa98xx->component, TFA98XX_AUDIO_CTR, val);
                        ...
                        struct snd_soc_dai *dai)
{
    ...
    basename = devm_kzalloc(tfa98xx->component->dev,
            MAX_CONTROL_NAME, GFP_KERNEL);
    if (!basename)
        return -ENOMEM;
...
static void __tfa98xx_irq(struct tfa98xx *tfa98xx)
{
    ...
    if (!(val & ie)) {
         ...
         * disabled
         */
        ireg = snd_soc_component_read32(tfa98xx->component,
                TFA98XX_INTERRUPT_REG);
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
    ...
    if (tfa98xx->reset_gpio == -EPROBE_DEFER)
        return -EPROBE_DEFER;
    ...
    if (tfa98xx->irq_gpio == -EPROBE_DEFER)
        return -EPROBE_DEFER;
...
static void tfa98xx_disable_regulator(void *data)
{
                 ...
                 const struct i2c_device_id *id)
{
    ...
    tfa98xx->tfa98xx_vdd = devm_regulator_get(&i2c->dev, "tfa9890_vdd");
    ...
    ret = devm_add_action_or_reset(&i2c->dev,
            tfa98xx_disable_regulator, tfa98xx->tfa98xx_vdd);
    if (ret)
        return ret;
...
static int __init tfa98xx_i2c_init(void)
{
	...
	return i2c_add_driver(&tfa98xx_i2c_driver);
}
